...
def test(desired_cluster, desired_far_away_from_cluster, nbr_contexts, d):
  ...
  ga = GeneticAlgorithm(nbr_variables=nbr_contexts, cluster=desired_cluster, cluster_complement=desired_far_away_from_cluster, d=d)
...
class GeneticAlgorithm(object):
  def __init__(self, nbr_variables, cluster, cluster_complement, d):
    ...
    self.tournamentSelectionParameter = 0.75
    self.numberOfGenerations = 2
    self.tournamentSize = 20
    self.p_crossover = 0.4
    self.p_mutation = 2 / self.nbr_variables
    self.p_creep = 8 * self.p_mutation
    self.best_index = 0
  ...
  def initialise_population(self):
    return (self.max_value - self.min_value) * np.random.rand(self.populationSize, self.nbr_variables) + self.min_value * np.ones([self.populationSize, self.nbr_variables])
