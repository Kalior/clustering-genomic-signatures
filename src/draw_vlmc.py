...
def save(tree_dir, out_dir):
  ...
  for vlmc in vlmcs:
    ...
    draw_with_probabilities(vlmc, metadata)
...
def draw_with_probabilities(vlmc, metadata):
  G = nx.DiGraph()
...
def add_children(vlmc, G, context, root_name):
  for c in vlmc.alphabet:
    ...
    if child in vlmc.tree:
      ...
      add_children(vlmc, G, child, root_name)
    else:
